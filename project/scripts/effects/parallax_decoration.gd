...
func _build_layers() -> void:
	...
	match theme:
		...
		ParallaxTheme.SNOW: _build_snow()
...
func _build_snow() -> void:
	_add_layer(Vector2(0.0, 0.0), Color(0.85, 0.88, 0.95), _draw_sky_gradient)
	_add_layer(Vector2(0.08, 0.0), Color(1.0, 1.0, 1.0, 0.6), _draw_snowflakes)
	_add_layer(Vector2(0.2, 0.05), Color(1.0, 1.0, 1.0, 0.4), _draw_snowflakes)
	_add_layer(Vector2(0.4, 0.15), Color(0.85, 0.9, 1.0, 0.5), _draw_snow_drifts)
...
func _draw_snowflakes(canvas: Control, color: Color) -> void:
	var rng := RandomNumberGenerator.new()
	rng.seed = _seed_value + 100 + int(_time * 5)
	var w := canvas.size.x
	var h := canvas.size.y

	for _s in range(30):
		var sx := rng.randf_range(0, w)
		var sy := fmod(rng.randf_range(0, h) + _time * 20, h + 100) - 50
		var sr := rng.randf_range(1.0, 3.5)
		canvas.draw_circle(Vector2(sx, sy), sr, color)

func _draw_snow_drifts(canvas: Control, color: Color) -> void:
	var rng := RandomNumberGenerator.new()
	...
	var x := 0.0
	while x <= w:
		var h := rng.randf_range(30, 100)
		var peak_x := x + rng.randf_range(40, 120)
		var next_x := x + rng.randf_range(80, 180)

		points.append(Vector2(x, base_y))
		points.append(Vector2(peak_x, base_y - h))
		x = next_x
